...
public class ObjectPoolManager : MonoBehaviour
{
    ...
    private void InitUIObjectPool()
    {
        ...
        Canvas canvas = m_uiObjectPool.AddComponent<Canvas>();
        if (canvas)
        {
        ...
        m_uiObjectPool.transform.SetParent(transform);
        m_uiObjectPool.transform.localPosition = Vector3.zero;
        m_uiObjectPool.transform.localScale = Vector3.one;
    }

    public void ReservePool(GameObject prefab, int size, bool isUIType = false)
    {
        ReservePool(prefab.name, prefab, size, isUIType);
    }
    ...
    public GameObject GetObject(string typeName, bool autoActive = true, bool isUIType = false)
    {
        ...
        // Find an inactive object in the list and pass it on
        for (int i = 0; i < objectList.Count; i++)
        {
            ...
            else if (objectList[i].activeSelf == false)
            {
                ...
                if (isUIType)
                    findObject.transform.position = new Vector3(4 + Random.Range(-1, 1), -9 + Random.Range(-1, 1), 0);
    ...
    public void HiddenAllUIPoolObjects()
    {
        foreach(Transform form in m_uiObjectPool.transform)
        {
